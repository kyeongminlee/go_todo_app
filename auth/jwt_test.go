...
func TestEmbed(t *testing.T) {
	...
	//data, _ := rawPrivKey.ReadFile("cert/secret.pem")
...
func TestJWTer_GenJWT(t *testing.T) {
	...
	user := fixture.User(&entity.User{ID: wantID})
	moq := &StoreMock{}
	...
	got, err := sut.GenerateToken(ctx, *user)
	if err != nil {
...
func TestJWTer_GetJWT(t *testing.T) {
	...
	privateKey, err := jwk.ParseKey(rawPrivKey, jwk.WithPEM(true))
	if err != nil {
		t.Fatal(err)
	}
...
func TestJWTer_GetJWT_NG(t *testing.T) {
	...
	for n, tt := range tests {
		t.Run(n, func(t *testing.T) {
			...
			got, err := sut.GetToken(ctx, req)
			if err == nil {
				t.Errorf("want no error, but got nil")
			}
			if got != nil {
